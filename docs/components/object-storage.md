...
Revocation: <short expiry, key/object changes, policy invalidation, or origin controls>
```
...
Already-issued signed URLs usually remain usable until expiry unless the
storage or delivery layer can reject them through key changes, object movement,
policy invalidation, or origin/CDN controls. Treat short TTLs as the primary
revocation tool for most private download paths.
...
flowchart LR
    ...
    Uploaded --> ProcessingJob[Queue job or object event]
    ProcessingJob --> Worker[Processing worker]
    ...
    Client -->|Download bytes with signed URL| ObjectStore
```
